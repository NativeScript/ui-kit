import { ItemEventData, CoreTypes, View, KeyedTemplate, Length, Color, Utils, StackLayout, ProxyViewContainer, Trace, Observable, EventData, Screen, Property } from '@nativescript/core';
import { ListViewBase, separatorColorProperty, itemTemplatesProperty, iosEstimatedRowHeightProperty } from '@nativescript/core/ui/list-view/list-view-common';
...
export const sectionsProperty = new Property<ListViewBase, any[]>({
  name: 'sections',
});
...
class ListViewCell extends UITableViewCell {
  ...
  public willMoveToSuperview(newSuperview: UIView): void {
    const parent = <ListViewCustom>(this.view ? this.view.parent : null);
...
class DataSource extends NSObject implements UITableViewDataSource {
  ...
  private _owner: WeakRef<ListViewCustom>;

  public static initWithOwner(owner: WeakRef<ListViewCustom>): DataSource {
  ...
  tableViewTitleForHeaderInSection(tableView: UITableView, section: number) {
    console.log('header section:', section);
    return this._owner.deref().sections?.[section]?.header || '';
  }

  tableViewTitleForFooterInSection(tableView: UITableView, section: number) {
    console.log('footer section:', section);
    return this._owner.deref().sections?.[section]?.footer || '';
  }

  numberOfSectionsInTableView(tableView: UITableView) {
    return this._owner.deref().sections?.length || 1;
  }

  public tableViewNumberOfRowsInSection(tableView: UITableView, section: number) {
    const owner = this._owner?.deref();

    return owner && owner.items ? (owner.sections?.length ? (owner.items as any).filter((i: any) => (typeof i.section === 'number' ? i.section === section : true)).length : owner.items.length) : 0;
  }
  ...
  public tableViewCellForRowAtIndexPath(tableView: UITableView, indexPath: NSIndexPath): UITableViewCell {
    ...
    if (owner) {
      const template = owner._getItemTemplate(indexPath.row, indexPath.section);
      ...
      if (cellView && cellView.isLayoutRequired) {
        ...
        const width = Utils.layout.getMeasureSpecSize(owner.widthMeasureSpec);
        ...
        // if (owner.listStyle === 'insetGrouped') {
        //   cellView.marginRight = owner.insetMarginRight;
        // }
        // const inset = Utils.layout.toDevicePixels(owner.insetMarginRight);
...
class UITableViewDelegateImpl extends NSObject implements UITableViewDelegate {
  ...
  public static initWithOwner(owner: WeakRef<ListViewCustom>): UITableViewDelegateImpl {
...
class UITableViewRowHeightDelegateImpl extends NSObject implements UITableViewDelegate {
  ...
  public static initWithOwner(owner: WeakRef<ListViewCustom>): UITableViewRowHeightDelegateImpl {
  ...
  public tableViewHeightForRowAtIndexPath(tableView: UITableView, indexPath: NSIndexPath): number {
    ...
    return Utils.layout.toDeviceIndependentPixels(owner._effectiveRowHeight);
  }
}

export type ListStyleType = 'insetGrouped' | 'grouped' | 'plain';

export class ListViewCustom extends ListViewBase {
  public nativeViewProtected: UITableView;
  listStyle: ListStyleType;
  sections: Array<{ id: number; header?: string; footer?: string }>;
  widthMeasureSpec = 0;
  insetMarginRight = 40;
  ...
  createNativeView() {
    ...
    switch (this.listStyle) {
      case 'insetGrouped':
        style = UITableViewStyle.InsetGrouped;
        break;
      case 'grouped':
    ...
    return UITableView.alloc().initWithFrameStyle(CGRectZero, style);
  }
   ...
   */
  // @ts-expect-error
  public _getItemTemplate(index: number, section: number): KeyedTemplate {
    let templateKey = 'default';
    if (this.itemTemplateSelector) {
      const dataItem = this._getDataItem(index, section);
      // @ts-expect-error
  ...
  // @ts-expect-error
  public _prepareItem(item: View, index: number, section: number) {
    if (item) {
      item.bindingContext = this._getDataItem(index, section);
    }
  }

  private _getDataItem(index: number, section: number): any {
    const thisItems = <any>this.items;
    const filter = (i: any) => {
      return this.sections?.length ? i.section === section : true;
    };

    console.log('Section:', section, ' item:', thisItems.filter(filter)[index]);
    return thisItems.getItem ? thisItems.filter(filter).getItem(index) : thisItems.filter(filter)[index];
  }
  /**
   * END OVERRIDEN
   */
  ...
  public _onRowHeightPropertyChanged(oldValue: CoreTypes.LengthType, newValue: CoreTypes.LengthType) {
    const value = Utils.layout.toDeviceIndependentPixels(this._effectiveRowHeight);
    const nativeView = this.nativeViewProtected;
    if (value < 0) {
      nativeView.rowHeight = UITableViewAutomaticDimension;
      nativeView.estimatedRowHeight = DEFAULT_HEIGHT;
  ...
  public onLayout(left: number, top: number, right: number, bottom: number): void {
    ...
    this._map.forEach((childView, listViewCell) => {
      ...
      if (cellHeight) {
        const width = Utils.layout.getMeasureSpecSize(this.widthMeasureSpec);
        childView.iosOverflowSafeAreaEnabled = false;
        // const inset = Utils.layout.toDevicePixels(this.insetMarginRight);
        // console.log('inset:', inset*3)
        // console.log(Screen.mainScreen.scale)
        // if (this.listStyle === 'insetGrouped') {
        //   console.log('childView.width:', childView.width)
        //   // @ts-expect-error
        //   childView.width = childView.width - inset;
        // }
  ...
  public _prepareCell(cell: ListViewCell, indexPath: NSIndexPath): number {
    ...
    try {
      ...
      if (!view) {
        view = this._getItemTemplate(indexPath.row, indexPath.section).createView();
      }
  ...
  public _removeContainer(cell: ListViewCell): void {
    ...
    if (!(view.parent instanceof ListViewCustom)) {
...
// @ts-expect-error
sectionsProperty.register(ListViewCustom);
